...
<hr>
      ...
      <h3>2022 Manu Express</h3>
      <div id="horizontal-two-items" class="IMGS IMGS_AVs">
        
        <div>
          <h3 style="margin-left:40px"></h3>
          <p>  Manu Express are day trains. This means, the train starts from the originating station, travels to its destination, and then returns to the originating station on the same day. Like Rajdhani and Duronto, Shatabdi Express trains are also superfast with a maximum speed of 120 km per hour. </p>
          <br>
          <p>Current Rate: <b>300.00 </b></p>
        </div>
      </div>
      <hr>
      <form action="saving_appointment.php?vehicle=Manu Express" method="POST" style="width:350px; padding:5px;margin:15px;margin-top:20px;">
        <fieldset>
          <legend>Book your train:</legend>
          <label for="startDate">Start Date: </label>
          <input type="hidden" value="HondaCivic">
...
$query = "Select * from reviews where vehicle = 'honda_civic'";
$result = $conn->query($query);
echo'<h3 style=" margin-left:520px; margin-top: -150px;"> REVIEWS...</h3>'
. '<div  style="overflow:scroll; margin-left:500px; margin-top: -40px;height:120px; width:350px;">'.'<ol>';
  while($row = $result->fetch_assoc()) {
  echo'<li>'.'<b>'.$row['username'].'</b>'.'<br>'  . $row['review'].'</li>'.'<br>';
}
echo'</ol>'.'</div>';
  echo'<form  action="insertreview.php?vehicle=honda_civic" method="POST">';
